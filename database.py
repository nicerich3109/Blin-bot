# -*- coding: utf-8 -*-
"""SQLite data/configuration layer for Blin Bot.

Discord object IDs are configuration data, never constants in source.
Persistent bot state is stored here; legacy JSON is only a migration source.
"""
import json
import os
import sqlite3
...
def add_warning(guild_id, target_id, issuer_id, reason, work_off, level):
    with connect() as db:
        db.execute("INSERT INTO disciplinary_warnings(guild_id,target_id,issuer_id,reason,work_off,level) VALUES(?,?,?,?,?,?)", (guild_id, target_id, issuer_id, reason, work_off, level))
