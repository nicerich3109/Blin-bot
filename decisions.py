...
def find_kind(key: str, guild_id: int | None = None):
    ...
    app = storage.DATA["applications"].get(key)
    if app is not None and (guild_id is None or app.get("guild_id", guild_id) == guild_id):
        return KIND_JOIN, key
    vac = storage.DATA["vacations"].get(key)
    if vac is not None and (guild_id is None or vac.get("guild_id", guild_id) == guild_id):
...
async def _decide_join(guild, staff_member, number, accepted, reason):
    ...
    if accepted and applicant is not None:
        ...
        try:
            await applicant.edit(nick=f"New | {server} | {first_word}", reason=f"Заявка {number} одобрена")
    ...
    if applicant is not None:
        ...
        if obzvon_role and obzvon_role in applicant.roles:
            try:
                ...
                logger.warning("Не удалось снять роль обзвона с %s", applicant.id)
    ...
    log_embed.add_field(name="Сервер", value=utils.server_name(guild.id, server), inline=True)
    log_embed.add_field(name="Заявитель", value=f"<@{app['applicant_id']}> ({app['applicant_id']})", inline=False)
    ...
    if ticket_channel:
        ...
        if applicant is not None:
            try:
                await ticket_channel.set_permissions(applicant, view_channel=False, send_messages=False, read_message_history=False)
            except discord.Forbidden:
                logger.warning("Не удалось закрыть доступ заявителю %s", applicant.id)
        ...
        if archive_category:
            try:
                await ticket_channel.edit(category=archive_category, reason=f"Заявка {number} обработана — перенос в архив")
            except discord.Forbidden:
                logger.error("Нет прав переместить канал заявки %s в архив", number)

    message = f"Заявка `{number}` обработана: {result_label}."
    if role_warning:
        message += f" Внимание: {role_warning}."
    return True, message
...
async def _decide_vacation(guild, staff_member, vac_id, accepted, reason):
    ...
    if vac is None or vac.get("guild_id", guild.id) != guild.id:
        return False, f"Заявка `{vac_id}` не найдена на этом сервере."
    ...
    if accepted:
        ...
        if role and target:
            try:
                ...
                role_warning = "у бота нет прав выдать роль отпуска"
        elif not role:
            role_warning = "роль отпуска не настроена в Dashboard"
        elif not target:
            role_warning = "участник не найден на сервере"
    logs_channel = guild.get_channel(utils.LOGS_CHANNELS[vac["server"]])
    await _finalize_vacation_log_message(guild, vac, vac_id, result_label, accepted, staff_member, reason, logs_channel)
    if accepted:
...
async def _finalize_vacation_log_message(guild, vac, vac_id, result_label, accepted, staff_member, reason, logs_channel):
    ...
    if channel and message_id:
        try:
            ...
            pass
    if logs_channel:
        embed = discord.Embed(title=f"Отпуск {vac_id} — {result_label}", color=discord.Color.green() if accepted else discord.Color.red())
        embed.add_field(name="Обработал", value=staff_member.mention, inline=True)
        if not accepted and reason:
            embed.add_field(name="Причина отказа", value=reason, inline=False)
