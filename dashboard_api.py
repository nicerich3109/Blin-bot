...
# OAuth state is kept server-side and mirrored in a short-lived HttpOnly cookie.
# The cookie makes the flow survive normal browser navigation, while the server-side
# value prevents accepting arbitrary state values.
OAUTH_STATES = set()
SESSIONS = {}
MANAGE_GUILD = 1 << 5
ADMINISTRATOR = 1 << 3
...
def create_app(bot):
    ...
    async def root(request):
        return web.json_response({"service": "Blin Bot API", "status": "online", "api_version": 5, "dashboard": "ready", "health": "/health"})

    async def health(request): return web.json_response({"ok": True, "service": "blin-bot", "api_version": 5})
    ...
    async def auth_callback(request):
        ...
        # Normal flow: Discord returns the exact state generated by /auth/discord.
        # Cookie fallback also handles browsers that omit the query parameter after
        # a navigation step, but never accepts a state that wasn't generated here.
    ...
    async def admin(request):
        ...
        return web.json_response({
            ...
            "configuration": {"configured_admin_count": len(config.SITE_ADMIN_IDS), "api_version": 5},
        })
