...
async def _publish_reaction(guild, config_id):
    ...
    view = RoleButtonView(guild.id, item.get("buttons", [])[:20])
    message = await _upsert_message(channel, f"reaction_roles_{guild.id}_{config_id}", content=None if embed else item.get("text"), embed=embed, view=view)
    return message, channel
    ...
    async def publish_panel(request):
        ...
        try:
            if section == "applications": message, channel = await _publish_applications(guild)
                ...
                stored_id = storage.DATA.get("persistent_messages", {}).get(f"vacation_info_{profile}")
                message = await channel.fetch_message(int(stored_id)) if stored_id else None
            ...
            logger.exception("Dashboard publish failed guild=%s section=%s", guild.id, section); return _error(f"discord_error_{exc.status}", 502)
