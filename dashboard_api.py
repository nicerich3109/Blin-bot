# -*- coding: utf-8 -*-
"""Dashboard API v2: runtime Discord data and per-guild configuration."""
import os
import discord
...
def create_app(bot):
    ...
    async def auth(request, handler):
        if request.path == "/health" or request.method == "OPTIONS": return await handler(request)
        if not _auth(request): return _error("unauthorized", 401)
        return await handler(request)

    app = web.Application(middlewares=[cors, auth])

    async def health(request): return web.json_response({"ok": True, "service": "blin-bot", "api_version": 2})
    async def guilds(request):
        return web.json_response([{"id": g.id, "name": g.name, "member_count": g.member_count} for g in bot.guilds if database.guild_enabled(g.id)])

    async def objects(request):
        guild = _guild(bot, request)
        if not guild: return _error("guild_not_found", 404)
        return web.json_response({
            "roles": [{"id": r.id, "name": r.name, "position": r.position, "managed": r.managed} for r in guild.roles if not r.managed],
    ...
    async def publish_contract(request):
        ...
        await publish_block(channel, block)
        return web.json_response({"ok": True, "channel_id": channel.id})
    ...
    async def provision(request):
        ...
        save_provisioning_result(guild.id, created)
    ...
    async def consent(request):
        ...
        if not guild: return _error("guild_not_found", 404)
        try:
            body = await request.json(); user_id = int(body["user_id"]); enabled = bool(body["enabled"])
    ...
    app.add_routes([
        web.get("/health", health), web.get("/api/guilds", guilds), web.get("/api/guilds/{guild_id}/objects", objects),
        web.get("/api/guilds/{guild_id}/config", cfg), web.put("/api/guilds/{guild_id}/config", cfg),
        ...
        web.post("/api/guilds/{guild_id}/contracts/{contract_id}/publish", publish_contract),
        web.post("/api/guilds/{guild_id}/provision", provision),
        web.get("/api/guilds/{guild_id}/warnings", warnings), web.get("/api/guilds/{guild_id}/reaction-roles", reaction_roles),
        web.post("/api/guilds/{guild_id}/reaction-roles", reaction_roles), web.put("/api/guilds/{guild_id}/consent", consent),
    ])
    return app
