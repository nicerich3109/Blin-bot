...
class RoleButtonView(discord.ui.View):
    def __init__(self,guild_id,buttons,scope_id=None):
        super().__init__(timeout=None); self.guild_id=guild_id; self.scope_id=scope_id
        scope=str(scope_id or "default")
        for index,item in enumerate(buttons):
            item=dict(item or {}); key=str(item.get("key") or index)
            b=discord.ui.Button(label=str(item.get("label","Роль"))[:80],style=getattr(discord.ButtonStyle,item.get("style","primary"),discord.ButtonStyle.primary),custom_id=f"role:{guild_id}:{scope}:{key}")
            async def callback(interaction,b=b,item=item):
                ...
                try:
                    if role in interaction.user.roles:
                        await interaction.user.remove_roles(role,reason="Blin Bot role button"); text=f"Роль {role.mention} снята."
                    else:
                        await interaction.user.add_roles(role,reason="Blin Bot role button"); text=f"Роль {role.mention} выдана."
...
async def publish(channel,settings,store=None,store_key=None):
    ...
    content=None if embed else settings.get("text"); message=None
    ...
    view=RoleButtonView(channel.guild.id,settings.get("buttons",[])[:20],settings.get("id"))
    if message:
