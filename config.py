...
# Заявка на отпуск теперь хранит точное время окончания с точностью до
# получаса (см. vacations.py), поэтому раз в час — слишком редко: роль
# могла бы провисеть лишний почти час после фактического окончания отпуска.
# Проверка каждые 10 минут (как и предлагалось в ТЗ) даёт точность +/-10
# минут. Мы используем 15 минут: этого достаточно, чтобы не отставать от
# получасовой сетки более чем на треть интервала, а нагрузка на API
# Discord (запрос списка каналов/участников + возможное редактирование
# сообщения со списком) при этом снижается на треть по сравнению с
# 10-минутным интервалом. Если нужна более высокая точность — верните 10.
VACATION_CHECK_INTERVAL_MINUTES = 15
