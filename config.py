...
# Как часто проверяется окончание отпусков и обновляется список (в минутах).
# Заявка на отпуск хранит точное время окончания с точностью до 10 минут
# (см. utils.parse_ten_minute_time), поэтому раз в час — слишком редко: роль
# могла бы провисеть лишний почти час после фактического окончания отпуска.
# Интервал синхронизирован с шагом времени в заявке (10 минут), чтобы не
# отставать от указанного времени больше чем на длину самого интервала.
# Если нагрузка на API Discord важнее точности — можно увеличить.
VACATION_CHECK_INTERVAL_MINUTES = 10
