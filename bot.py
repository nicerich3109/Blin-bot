...
class BlinBot(commands.Bot):
    async def setup_hook(self):
        database.init_db()
        self.add_view(JoinInfoView()); self.add_view(VacationInfoView("DN")); self.add_view(VacationInfoView("PHX"))
...
async def on_ready():
    ...
    for guild in bot.guilds:
        database.register_guild(guild.id,guild.name); utils.refresh_runtime_config(guild.id)
        raw=database.get_config(guild.id); channel_id=raw.get("recruit_info_channel")
        ...
        if recruit_channel:
            ...
            await utils.ensure_persistent_message(recruit_channel,storage.DATA,f"recruit_info_{guild.id}",[embed],JoinInfoView()); await storage.persist()
