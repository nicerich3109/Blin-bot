...
class BlinBot(commands.Bot):
    async def setup_hook(self):
        ...
        try:
            ...
            raise
        try:
            await self.tree.sync(); logger.info("Discord slash-команды синхронизированы")
...
async def restore_dashboard_views():
    """Register persistent views for panels published from Dashboard."""
        ...
        try:
            ...
            if profiles:
                ...
                for profile in profiles:
                    if database.get_server_config(guild.id, profile).get("vacation_channel"):
                        bot.add_view(VacationInfoView(profile))
            for item in database.list_reaction_role_configs(guild.id):
                bot.add_view(RoleButtonView(guild.id, item.get("buttons", [])[:20]))
            for block in database.list_contracts(guild.id):
                if block.get("channel_id"):
                    _, _, view = _build_publish_payload(block)
                    bot.add_view(view)
        except Exception:
            logger.exception("Не удалось восстановить Dashboard views для guild=%s", guild.id)
...
async def on_ready():
    ...
    for guild in bot.guilds:
        ...
        server_profiles = database.server_configs(guild.id)
        recruit_channel_id = raw.get("recruit_info_channel")
        recruit_channel = guild.get_channel(int(recruit_channel_id)) if recruit_channel_id else None
        if recruit_channel:
