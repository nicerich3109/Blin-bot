...
class ConsentView(discord.ui.View):
    def __init__(self, continuation): super().__init__(timeout=180); self.continuation=continuation
    @discord.ui.button(label="Принять",style=discord.ButtonStyle.success)
    async def accept(self, interaction, button):
        await consent_storage.set_consent(interaction.user.id,True)
        await interaction.response.edit_message(content="✅ Согласие сохранено. Заявка продолжается.",view=None)
